...
if (real?.numeroProcesso) {
  const byNum = await client.callTool({
    name: "inpi_search_by_process_number",
    arguments: { numero_processo: real.numeroProcesso },
  });
...
// --- inpi_search_by_mark_advanced ---
const adv = await client.callTool({
  name: "inpi_search_by_mark_advanced",
  arguments: { marca: "GOOGLE", busca_fuzzy: false, apresentacao: "qualquer", natureza: "qualquer", apenas_pedidos_vivos: true, resultados_por_pagina: 20 },
});
...
const owner1 = await client.callTool({ name: "inpi_search_by_owner", arguments: { nome: "GOOGLE", resultados_por_pagina: 20 } });
...
if (Array.isArray(sOwner1?.titularesCandidatos) && sOwner1.titularesCandidatos.length > 0) {
  ...
  const owner2 = await client.callTool({ name: "inpi_search_by_owner", arguments: { nome: "GOOGLE", pos, resultados_por_pagina: 20 } });
...
const fig = await client.callTool({
  name: "inpi_search_by_figurative_code",
  arguments: { viena_1: "26.04.01", resultados_por_pagina: 20 },
});
...
const markAgain = await client.callTool({
  name: "inpi_search_by_mark",
  arguments: { marca: "GOOGLE", busca_exata: true, resultados_por_pagina: 20 },
});
const sMarkAgain = markAgain.structuredContent;
if (sMarkAgain?.totalPaginas > 1) {
  const next = await client.callTool({ name: "inpi_next_page", arguments: { pagina: 2 } });
...
const htmlCall = await client.callTool({ name: "inpi_search_by_mark", arguments: { ...cacheArgs, salvar_html: true, forcar_atualizacao: true } });
...
const cachedCall = await client.callTool({ name: "inpi_search_by_mark", arguments: cacheArgs });
...
const forcedCall = await client.callTool({ name: "inpi_search_by_mark", arguments: { ...cacheArgs, forcar_atualizacao: true } });
